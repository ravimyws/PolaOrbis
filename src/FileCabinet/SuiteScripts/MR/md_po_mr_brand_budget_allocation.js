 ...
 */
define(["N/query", "N/record", "N/runtime", "N/email", "N/url"], /**
 * @param{query} query
 * @param{record} record,
 * @param{runtime} runtime
 * @param{email} email
 */ (query, record, runtime, email, url) => {
  /**
  ...
  const getInputData = (inputContext) => {
    ...
    try {
      ...
      let requestData = budgetGateWay.getRequestData();
      ...
      let brand = rec.getValue("custrecord_md_po_bdgt_allo_brand");

      let year = rec.getValue("custrecord_md_po_bdgt_allo_bdgt_year");

      if (!(brand && year)) {
        throw "Invalid Data";
      }

      let allocationRecords = budgetGateWay.getAllocationRecordsByBrand(
        brand,
        year
      );
      ...
      let brandBudgetIds = budgetGateWay.getBudgetIdsByBrand(
        brand,
        year,
        allocationRecords[0].subsidiary
      );
    ...
    try {
      ...
      if (budgetId) {
        ...
        mapContext.write({
          key: "update",
          value: {
            details: { id: budgetId, key: mapContext.key },
            type: "update",
          },
        });
      } else {
        ...
        mapContext.write({
          key: "create",
          value: {
            details: { id: createId, key: mapContext.key },
            type: "create",
          },
        });
      }
      ...
      mapContext.write({
        key: "error",
        value: {
          details: { error: e, budget: budget.b, key: mapContext.key },
          type: "error",
        },
      });
    }
  ...
  const reduce = (reduceContext) => {
    try {
      reduceContext.write({
        ...
        value: reduceContext.values.map((e) => {
          return JSON.parse(e);
        }),
      });
    } catch (e) {
  ...
  const summarize = (summaryContext) => {
    ...
    summaryContext.output.iterator().each(function (key, value) {
      ...
      log.error(key, cvalue);
      let transformValue = cvalue.map((e) => {
        return e.details.id ? e.details.id : e.details;
      });

      data[key] = transformValue;
    ...
    if (requestData) {
      ...
      let emailId = rec.getValue("custrecord_md_po_bdgt_allo_email");
      ...
      record.submitFields({
        ...
        id: requestData,
        values: {
          custrecord_md_po_bdgt_allo_mr_status: status,
          custrecord_md_po_bdgt_allo_proc_log: JSON.stringify(data),
        },
      });
      if (emailId && status === "failed") {
        let recUrl = url.resolveRecord({
          recordType: "customrecord_md_po_bdgt_allo_scr_rec",
          recordId: requestData,
        });
        email.send({
          author: -5,
          recipients: emailId,
          subject: "Budget Allocation MR Error Notification ",
          body: `<html><body>Please check this record for <a href="https://system.netsuite.com${recUrl}">more details</a></body></html>`,
        });
      }
    }
  };
  ...
  class BudgetGateWay {
    ...
    getRequestData() {
      let currentScript = this.runtime.getCurrentScript();
      let brand = currentScript.getParameter({
        name: "custscript_md_po_mr_bdgt_allo_req_data",
      });
      return brand;
    }
    ...
    getBudgetByBodyFields(bodyFields) {
      let sql = `select id  from budgets b
              ...
              and  nvl(b.department,1) = nvl(${
                bodyFields.department || null
              },1) 
    ...
    getAllocationRecordsByBrand(brand, year) {
      let sql = `select 
                        distinct als.name, 
                        als.id ,
                        als.subsidiary
                      from 
                        ...
                        Where b.class = ? and ap.id =?`;

      return this.getResults(sql, [brand, year]);
    }
    ...
    getBudgetIdsByBrand(brand, year, subsidiary) {
      let sql = `select id from budgets where class = ?  and year=? and subsidiary =?`;

      return this.getResults(sql, [brand, year, subsidiary]);
    }
